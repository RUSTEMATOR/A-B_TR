...
export default class RecursionsStageNL {
    constructor(){}

    async recursiveTestStageGoddesNL(stageLink: string): Promise<any> {
        ...
        if (baseCurrentUrl === EXPECTED_NETHERLANDS_STAGE_LINKS.goddes){
            ...
            methods.sleep(1000)
            await ctx.close();
            return this.recursiveTestStageGoddesNL(stageLink);
        }
    }

    async recursiveTestStagePharaohNL(stageLink: string): Promise<any> {
        ...
        if (baseCurrentUrl === EXPECTED_NETHERLANDS_STAGE_LINKS.pharaoh){
            ...
            methods.sleep(1000)
            await ctx.close();
            return this.recursiveTestStagePharaohNL(stageLink);
        }
    }
}