...
export default class RecursionsStageAT {
    constructor(){}

    async recursiveTestStageGoddesAT(stageLink: string): Promise<any> {
        ...
        if (baseCurrentUrl === EXPECTED_AUSTRIA_STAGE_LINKS.goddes){
            ...
            methods.sleep(1000)
            await ctx.close();
            return this.recursiveTestStageGoddesAT(stageLink);
        }
    }

    async recursiveTestStagePharaohAT(stageLink: string): Promise<any> {
        ...
        if (baseCurrentUrl === EXPECTED_AUSTRIA_STAGE_LINKS.pharaoh){
            ...
            methods.sleep(1000)
            await ctx.close();
            return this.recursiveTestStagePharaohAT(stageLink);
        }
    }
}