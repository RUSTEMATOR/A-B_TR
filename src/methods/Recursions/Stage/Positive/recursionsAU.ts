...
import { EXPECTED_AUSTRALIA_STAGE_LINKS } from "../../../../Data/AU/expectedAustraliaLinks";
...
export default class RecursionsStageAU {
    constructor(){}

    async recursiveTestStageGoddesAU(stageLink: string): Promise<any> {
        ...
        if (baseCurrentUrl === EXPECTED_AUSTRALIA_STAGE_LINKS.goddes){
            const baseCurrentUrl = await methods.formBaseLink()
            const receivedParameters = await methods.formQueryParameters()

            await methods.checkUrl(baseCurrentUrl, EXPECTED_AUSTRALIA_STAGE_LINKS.goddes, EXPECTED_AUSTRALIA_STAGE_LINKS.pharaoh)
            await methods.checkQueryParameters(receivedParameters, EXPECTED_QUERY.expctedQueryAU)

            await page.waitForTimeout(1000)

            
            await regMethods.makeFullScreenshot({fullPage: true, path: `Screenshots/GoddesAU.png`})
            ...
            qase.comment(`Registered with: ${randomEmail}\n\n
                ...
                Current URL: ${baseCurrentUrl}\n Expected links: \n${EXPECTED_AUSTRALIA_STAGE_LINKS.goddes}\n${EXPECTED_AUSTRALIA_STAGE_LINKS.pharaoh}
            ...
            methods.sleep(1000)
            await ctx.close();
            return this.recursiveTestStageGoddesAU(stageLink);
        }
    }

    async recursiveTestStagePharaohAU(stageLink?: string): Promise<any> {
        ...
        if (baseCurrentUrl === EXPECTED_AUSTRALIA_STAGE_LINKS.pharaoh){
            const baseCurrentUrl = await methods.formBaseLink()
            const receivedParameters = await methods.formQueryParameters()

            await methods.checkUrl(baseCurrentUrl, EXPECTED_AUSTRALIA_STAGE_LINKS.pharaoh, EXPECTED_AUSTRALIA_STAGE_LINKS.goddes)
            await methods.checkQueryParameters(receivedParameters, EXPECTED_QUERY.expctedQueryAU)

            await page.waitForTimeout(1000)

            
            await regMethods.makeFullScreenshot({fullPage: true, path: `Screenshots/PharaohAU.png`})
            ...
            methods.sleep(1000)
            await ctx.close();
            return this.recursiveTestStagePharaohAU(stageLink);
        }
    }
}