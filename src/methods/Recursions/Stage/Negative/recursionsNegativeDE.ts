...
export default class RecursionsNegativeStageDE {
    constructor(){}

    async recursiveNegativeTestStageGoddesDE(wrongEmail: string, stageLink: string): Promise<any> {
        ...
        if (baseCurrentUrl === EXPECTED_GERMANY_STAGE_LINKS.goddes){
            ...
            methods.sleep(1000)
            await ctx.close();
            return this.recursiveNegativeTestStageGoddesDE(wrongEmail, stageLink);
        }
    }

    async recursiveNegativeTestStagePharaohDE(wrongEmail: string, stageLink: string): Promise<any> {
        ...
        if (baseCurrentUrl === EXPECTED_GERMANY_STAGE_LINKS.pharaoh){
            ...
            methods.sleep(1000)
            await ctx.close();
            return this.recursiveNegativeTestStagePharaohDE(wrongEmail, stageLink);
        }
    }
}