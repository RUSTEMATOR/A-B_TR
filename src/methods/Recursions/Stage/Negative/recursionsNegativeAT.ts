...
import { EXPECTED_AUSTRIA_STAGE_LINKS } from "../../../../Data/AT/expectedAustriaLinks";
...
export default class RecursionsNegativeStageAT {
    constructor(){}

    async recursiveNegativeTestStageGoddesAT(wrongEmail: string, stageLink: string): Promise<any> {
        ...
        if (baseCurrentUrl === EXPECTED_AUSTRIA_STAGE_LINKS.goddes){
            const baseCurrentUrl = await methods.formBaseLink()
            const receivedParameters = await methods.formQueryParameters()

            await methods.checkUrl(baseCurrentUrl, EXPECTED_AUSTRIA_STAGE_LINKS.goddes, EXPECTED_AUSTRIA_STAGE_LINKS.pharaoh)
            ...
            qase.comment(`Registered with: ${wrongEmail}\n\n
                ...
                Current URL: ${baseCurrentUrl}\n Expected links: \n${EXPECTED_AUSTRIA_STAGE_LINKS.goddes}\n${EXPECTED_AUSTRIA_STAGE_LINKS.pharaoh}
            ...
            methods.sleep(1000)
            await ctx.close();
            return this.recursiveNegativeTestStageGoddesAT(wrongEmail, stageLink);
        }
    }

    async recursiveNegativeTestStagePharaohAT(wrongEmail: string, stageLink?: string): Promise<any> {
        ...
        if (baseCurrentUrl === EXPECTED_AUSTRIA_STAGE_LINKS.pharaoh){
            const baseCurrentUrl = await methods.formBaseLink()
            const receivedParameters = await methods.formQueryParameters()

            await methods.checkUrl(baseCurrentUrl, EXPECTED_AUSTRIA_STAGE_LINKS.pharaoh, EXPECTED_AUSTRIA_STAGE_LINKS.goddes)
            await methods.checkQueryParameters(receivedParameters, EXPECTED_QUERY.expctedQueryAT)
            ...
            methods.sleep(1000)
            await ctx.close();
            return this.recursiveNegativeTestStagePharaohAT(wrongEmail, stageLink)
        }
    }
}