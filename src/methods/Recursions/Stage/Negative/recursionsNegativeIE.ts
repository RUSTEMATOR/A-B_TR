...
export default class RecursionsNegativeStageIE {
    constructor(){}

    async recursiveNegativeTestStageGoddesIE(wrongEmail: string, stageLink: string): Promise<any> {
        ...
        if (baseCurrentUrl === EXPECTED_IRELAND_STAGE_LINKS.goddes){
            ...
            methods.sleep(1000)
            await ctx.close();
            return this.recursiveNegativeTestStageGoddesIE(wrongEmail, stageLink);
        }
    }

    async recursiveNegativeTestStagePharaohIE(wrongEmail: string, stageLink: string): Promise<any> {
        ...
        if (baseCurrentUrl === EXPECTED_IRELAND_STAGE_LINKS.pharaoh){
            ...
            methods.sleep(1000)
            await ctx.close();
            return this.recursiveNegativeTestStagePharaohIE(wrongEmail, stageLink);
        }
    }
}